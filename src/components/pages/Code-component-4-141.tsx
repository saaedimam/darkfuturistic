...
import { ArrowRight, Zap, Shield, Rocket, Target, ChevronUp, Star } from "lucide-react"
...
export function ScrollLandingPage() {
  ...
  const featureRefs = useState([])
  const testimonialRefs = useState([])
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              const ref = featureRefs[index] || React.createRef()
              const isVisible = featureRefs[index]
                ? featureRefs[index].current &&
                  featureRefs[index].current.getBoundingClientRect().top < window.innerHeight
                : false
...
function StickyTestimonials({ refs }) {
  const testimonials = [
    {
      name: "Sarah Chen",
  ...
  return (
    <div className="space-y-8">
      {testimonials.map((testimonial, index) => {
        const ref = refs[index] || React.createRef()
        const isVisible = refs[index]
          ? refs[index].current && refs[index].current.getBoundingClientRect().top < window.innerHeight
          : false
